...
VelodyneDriver::VelodyneDriver(ros::NodeHandle node, ros::NodeHandle private_nh)
{
  ...
  } else if (config_.model == "VLP32") {
    packet_rate = 1507;
    model_full_name = "VLP-32";
  } else {
...
bool VelodyneDriver::poll(void)
{
  ...
  // Here we assume we know the exact number of packets based on fixed rpm variable (used to
  // calculate freq. then npackets!)
  // Actually the manual states that the motor will diff with +- 3RPM (page 51 of VLP-32C manual)
  scan->packets.resize(config_.npackets);

  //TODO We should instead count the accumulated azimuth of packets, and publish once we
  //have enough.
