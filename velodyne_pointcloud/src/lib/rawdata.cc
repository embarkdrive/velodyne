 ...
 */
float RawData::unpack(const velodyne_msgs::VelodynePacket& pkt, VPointCloud& pc)
{
...
// TODO, I see no reason to not also report the absolute azimuth (begin,end)
...
float RawData::unpack_vlp(const velodyne_msgs::VelodynePacket& pkt, VPointCloud& pc)
{
  ...
  for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
    ...
    if (block < (BLOCKS_PER_PACKET - 1)) {
      // Wait a minute here... u_int16 - u_int16 where the later one could be larger than the first?
      // azimuth_diff = (float)((36000 + raw->blocks[block+1].rotation -
      // raw->blocks[block].rotation)%36000);

      azimuth_diff = (float)((36000 + static_cast<int>(raw->blocks[block + 1].rotation) -
                              static_cast<int>(raw->blocks[block].rotation)) %
                             36000);
    ...
    for (int firing_seq = 0, k = 0; firing_seq < vlp_spec_.firing_seqs_per_block; firing_seq++) {
      for (int laser = 0; laser < vlp_spec_.lasers_per_firing_seq; laser++, k += RAW_SCAN_SIZE) {
        velodyne_pointcloud::LaserCorrection& corrections = calibration_.laser_corrections[laser];
        ...
        // This is wrong I think, the manual states that the lasers are fired at the same time
        ...
        // Azimuth is reported as 100th of an angle
